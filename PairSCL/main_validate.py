...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='SNLI',
                        choices=['SNLI', 'MNLI', 'EXIM', 'SKILL', 'SKILL_Q', 'Hotpot'], help='dataset')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset in ['SNLI', 'EXIM', 'SKILL', 'SKILL_Q']:
        ...
        validate_loader = torch.utils.data.DataLoader(
            validate_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True,
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            loss, train_acc = train(train_loader, model, classifier, criterion, optimizer, epoch, args)
            time2 = time.time()
            print('epoch {}, total time {:.2f}, loss {:.2f}, accuracy {:.2f}'.format(epoch, time2 - time1, loss,
                                                                                     train_acc))
...
def train(train_loader, model, classifier, criterion, optimizer, epoch, args):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        labels = batch[3]
        loss = criterion(logits.view(-1, args.label_num), labels.view(-1))
