        ...
        with open(filepath, "r", encoding="utf8") as f:
            ...
            for example in input_data:
                ...
                if self.ignore_punctuation:
                    question = question.translate(punct_table)
                    context = context.translate(punct_table)

                # Each premise and hypothesis is split into a list of words.
                questions.append([w for w in question.rstrip().split()
                                 if w not in self.stopwords])
                contexts.append([w for w in context.rstrip().split()
                                   if w not in self.stopwords])
                labels.append(label)
                ids.append(pair_id)
