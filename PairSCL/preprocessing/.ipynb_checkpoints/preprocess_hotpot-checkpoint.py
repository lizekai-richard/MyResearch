...
def preprocess_SNLI_data(inputdir,
    ...
    for file in os.listdir(inputdir):
        if fnmatch.fnmatch(file, "*_train.json"):
            train_file = file
        elif fnmatch.fnmatch(file, "*_dev.json"):
            dev_file = file
...
if __name__ == "__main__":
    default_config = "../config/hotpot_preprocessing.json"

    parser = argparse.ArgumentParser(description="Preprocess the Hotpot dataset")
    parser.add_argument(
        "--config",
        default=default_config,
        help="Path to a configuration file for preprocessing Hotpot"
    )
